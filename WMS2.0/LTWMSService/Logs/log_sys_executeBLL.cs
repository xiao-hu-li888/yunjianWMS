...
{
    public class log_sys_executeBLL : LTWMSEFModel.ComDao<LTWMSEFModel.Logs.log_sys_execute>
    {
        public log_sys_executeBLL(LTWMSEFModel.LTModel dbContext) : base(dbContext)
        {
        }
        /// <summary>
        /// 删除半年前的历史数据
        ...
        { 
            ...
            return dbcontext.Database.ExecuteSqlCommand(
                   "delete from log_sys_execute where log_date < @log_date", sqlParameters) > 0 ? true : false;
        }
        /// <summary>
        /// 添加日志，超过长度500 分多次添加
        /// </summary>
        ...
        /// <returns></returns>
        public LTWMSEFModel.SimpleBackValue Add(string log, int diff)
        {
            ...
            List<LTWMSEFModel.Logs.log_sys_execute> loglist = new List<LTWMSEFModel.Logs.log_sys_execute>();
            for (int i = 0; i < _count; i++)
            {
                ...
                loglist.Add(new LTWMSEFModel.Logs.log_sys_execute()
                {
