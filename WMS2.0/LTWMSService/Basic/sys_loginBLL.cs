...
{
    public class sys_loginBLL : LTWMSEFModel.ComDao<LTWMSEFModel.Basic.sys_login>
    {

        sys_loginroleBLL bll_loginrole;
        sys_roleBLL bll_sysrole;
        public sys_loginBLL(LTWMSEFModel.LTModel dbContext) : base(dbContext)
        {
        ...
        /// <returns></returns>
        public LTWMSEFModel.SimpleBackValue AddRoles(string ex_role,Guid login_guid)
        {
            //删除关联的角色
           // bll_loginrole.Delete(w => w.loginguid == login_guid);
            ...
            //添加角色
            string rols = "," + ex_role + ",";
            var rollist = bll_sysrole.GetAllQuery(w => w.state == LTWMSEFModel.EntityStatus.Normal);
       ...
       /// <returns></returns>
        ...
        public SimpleBackValue UpdatePassword(Guid guid, string newpass)
        {
            ...
            {
                _us.password = npd;
                _us.updatedate = DateTime.Now;//通过修改时间判断最后一次密码修改时间。。。
                return Update(_us);
            }
            return SimpleBackValue.False;
        }
