...
{
    public class sys_table_idBLL : LTERPEFModel.ComDao<LTERPEFModel.Basic.sys_table_id>
    {
        public sys_table_idBLL(LTERPEFModel.LTModel dbContext) : base(dbContext)
        {
        ...
        public int GetId(TableIdType table, int take)
        {
            ...
            if (_obj != null)
            {
                ...
                LTERPEFModel.SimpleBackValue bv = Update(_obj);
                if (bv == LTERPEFModel.SimpleBackValue.DbUpdateConcurrencyException)
                {//如果出现并发异常，重新获取数据，重新修改一次
                    return GetId(table, take);
                }
                else if (bv == LTERPEFModel.SimpleBackValue.True)
                {
                    count = 0;
                    return _rtv;
                } 
            ...
            else
            {
                ...
                LTERPEFModel.SimpleBackValue bv = AddIfNotExists(new LTERPEFModel.Basic.sys_table_id()
                {
                    guid = Guid.NewGuid(),
                ...
                if (bv == LTERPEFModel.SimpleBackValue.True)
                {//初始化数据返回1
                    count = 0;
                    return _rtv;
                }
