...
namespace LTERPWebMVC.App_Start.AppCode
{
    public class MenuHelper
    {
        ...
        public static bool HasPermission(LTERPService.Basic.sys_loginBLL bll_login, Guid currloginguid, string _controller, string _action)
        {//判断当前用户是否是超级管理员
            ...
            List<LTERPEFModel.Basic.sys_role> roleList = bll_login.GetLoginRoles(currloginguid);
            if (roleList != null)
            {
        ...
        public static bool IsPremission(List<LTERPEFModel.Basic.sys_role> roleList, string premission)
        {
        ...
        public static MenuData GetCurrentLoginMenu(LTERPService.Basic.sys_loginBLL bll_login, Guid currLoginGuid)
        {
            //判断当前用户是否是超级管理员
            if (bll_login.GetAny(w => w.guid == currLoginGuid && w.issuperadmin == true))
            {
            ...
            var lstRole = bll_login.GetLoginRoles(currLoginGuid).Where(w => w.active == true && w.state == LTERPEFModel.EntityStatus.Normal).ToList();
